...
<div class="content">
	<div class="padding">
		<div class="bgwhite">
			...
			<div class="contenttop">
				<div class="left">
					<h3 class="jdl">Laporan Transaksi</h3>
				</div>
				<div class="right">
					<script type="text/javascript">
					function gotocat(val){
							var value=val.options[val.selectedIndex].value;
							window.location.href="laporan.php?id_cat="+value+"";
						}
	...
	<select class="leftin1" onchange="gotocat(this)">
						...
						<?php
							...
							$data=$root->con->query("select * from user where id_lokasi='$id_lokasi'");
							while ($f=$data->fetch_assoc()) {
								?>
									<option <?php if (isset($_GET['id_cat'])) { if ($_GET['id_cat'] == $f['id']) { echo "selected"; } } ?> value="<?= $f['id'] ?>"><?= $f['nama'] ?></option>
								<?php
							}
						?>
					</select>
    <form action="cetak_laporan.php" style="display: inline;" target="_blank" method="post">
			...
			<tbody>
					...
					$root->tampil_pegawai_filter($_GET['id_cat']);
					}
