				...
				<div class="right">
					<script type="text/javascript">
						function gotocat(val){
							...
							window.location.href="laporan_owner.php?id_cat="+value+"";
						}
						function gotojenis(val){
							var value=val.options[val.selectedIndex].value;
							window.location.href="laporan_owner.php?jenis="+value+"";
						}
						function gotofilter(val){
							var value=val.options[val.selectedIndex].value;
							
							window.location.href="laporan_owner.php?jenis=<?php if (isset($_GET['jenis'])) {
								echo $_GET['jenis'];
							} ?>&filter_record="+value;
						}
					</script>
    ...
    <form action="cetak_laporan.php" style="display: inline;" target="_blank" method="post">
			...
			<tbody>
					...
					if (isset($_GET['id_cat']) && $_GET['id_cat']) {
						$root->tampil_lokasi_filter_owner($_GET['id_cat']);
					}
					else if (isset($_GET['filter_record'])) {
						if ($_GET['jenis']=='perhari') {
							$aksi1=1;
						}else{
							$aksi1=2;
						}
						$root->filter_tampil_laporan_owner($_GET['filter_record'],$aksi1);
					}else{
					$root->tampil_laporan_owner();
					}
					?>
</tbody>
